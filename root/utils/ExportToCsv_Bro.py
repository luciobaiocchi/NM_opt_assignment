...
def process_batch(writer, method, gradient_fn, hessian_fn, N, h, dynamic, include_params, convergence_tail=None):
    ...
    for idx, x in enumerate(hypercube_bro):
        # --- AVVIO TIMER ---
...
def plot_single_config(N, h, dynamic, histories_dict, log, save_filename):
    ...
    histories_dict mappa label -> history per ogni punto di partenza.
    log è la stringa descrittiva dell'esperimento (es: "4. MN, All Approx (FD)").
    """
...
def run_experiment(log, filename, method, gradient_fn, hessian_fn, fieldnames, use_h_dynamic_loops=True, convergence_tail=None, generate_plot=False, is_exact=False):
    """
    ...
    OUTPUT_DIR = f"./plots_summary/Bro/{log_clean}"
                ...
                for h in h_values:
                    for dynamic in dynamic_h:
                        if verbose:
                            ...
                            print(f"==================  {h, N, dynamic}  ==================")
                            print('\n')
                        s, t, histories = process_batch(writer, method, gradient_fn, hessian_fn, N, h, dynamic, True, convergence_tail)
                        total_successes += s
                        total_attempts += t
                        ...
                        if enable_plotting and generate_plot and histories:
                            ...
                            fname = f"{OUTPUT_DIR}/Bro_N{N}_h{h_str}_{dynamic_str}.png"
                            plot_single_config(N, h, dynamic, histories, log, fname)
        else:
            # Esperimento senza h/dynamic loops (Exact)
            for N in n_list:
                s, t, histories = process_batch(writer, method, gradient_fn, hessian_fn, N, None, None, False, convergence_tail)
                total_successes += s
                total_attempts += t
                
                # Genera plot per Exact (no h/dynamic)
                if enable_plotting and generate_plot and histories:
                    fname = f"{OUTPUT_DIR}/Bro_N{N}_exact.png"
...
if __name__ == "__main__":
    ...
    p1 = Process(target=run_experiment, kwargs={
        ...
        "filename": "./results/results_tn_broyden_exact_gradient.csv",
        "method": NewtonMethods.truncated_newton,
        "gradient_fn": BroydenProblem.gradient_exact,
        "hessian_fn": BroydenProblem.hessian_with_jacobian,
    ...
    p2 = Process(target=run_experiment, kwargs={
        ...
        "filename": "./results/results_mn_broyden_exact_gradient.csv",
        "method": NewtonMethods.modified_newton_bro,
        "gradient_fn": BroydenProblem.gradient_exact,
        "hessian_fn": BroydenProblem.hessian_with_jacobian,
        "fieldnames": fieldnames_full,
        "use_h_dynamic_loops": True,
        "convergence_tail": None,
        "generate_plot": True,
        "is_exact": False
    })
    processes.append(p2)
    ...
    p3 = Process(target=run_experiment, kwargs={
        ...
        "filename": "./results/results_tn_broyden_all_aprox.csv",
        "method": NewtonMethods.truncated_newton,
        "gradient_fn": BroydenProblem.gradient_fd,
        "hessian_fn": BroydenProblem.hessian_fd,
        "fieldnames": fieldnames_full,
        "use_h_dynamic_loops": True,
        "convergence_tail": None,
        "generate_plot": True,
    ...
    p4 = Process(target=run_experiment, kwargs={
        ...
        "filename": "./results/results_mn_broyden_all_aprox.csv",
        "method": NewtonMethods.modified_newton_bro,
        "gradient_fn": BroydenProblem.gradient_fd,
        "hessian_fn": BroydenProblem.hessian_fd,
        "fieldnames": fieldnames_full,
    ...
    p5 = Process(target=run_experiment, kwargs={
        ...
        "filename": "./results/results_tn_broyden_all_exact.csv",
        "method": NewtonMethods.truncated_newton,
        "gradient_fn": BroydenProblem.gradient_exact,
        "hessian_fn": BroydenProblem.hessian_exact,
    ...
    p6 = Process(target=run_experiment, kwargs={
        ...
        "filename": "./results/results_mn_broyden_all_exact.csv",
        "method": NewtonMethods.modified_newton_bro,
        "gradient_fn": BroydenProblem.gradient_exact,
        "hessian_fn": BroydenProblem.hessian_exact,
        "fieldnames": fieldnames_simple,
